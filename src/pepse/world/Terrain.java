...
/**
 * The class handles all the ground and blocks, as well as providing
 * information regarding the terrain height.
 */
public class Terrain {

    private static final float GROUND_HEIGHT_PARAM = 4.5f / 6;
    private static final String GROUND_TAG = "ground";
    private static final float HEIGHT_AT_X0_FACTOR = 0.66f;
     ...
     *
     * @param gameObjects      - The collection to
     *                         which we add the terrain.
     * @param groundLayer      - The layer to which
     *                         the terrain is added.
     * @param windowDimensions - The dimensions of
     *                         the game window.
     * @param seed             - Integer Parameter
    ...
    private float calcGroundHeightAt(float x) {
        return windowDimensions.y() * GROUND_HEIGHT_PARAM +
                noise.noise(x);
    }

    /**
     * Determines what is the desired ground height at a given
     * point, using additional logic to prevent
    ...
    public float groundHeightAt(float x) {
        ...
        if (x - Block.SIZE > Block.SIZE && x + Block.SIZE <
                windowDimensions.x()) {
            if (calcGroundHeightAt(x - Block.SIZE) <
                    result + Block.SIZE
                    && calcGroundHeightAt(x + Block.SIZE) <
                    result + Block.SIZE)
    ...
    public void createInRange(int minX, int maxX) {
        ...
        for (int i = minX; i <= maxX; i += Block.SIZE) {
            ...
            ground.setDimensions(new Vector2(Block.SIZE,
                    Block.SIZE * TERRAIN_DEPTH));
